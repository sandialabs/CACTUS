...
If \path{WakeElemOutFlag} = 1, information about each vortex filament is written to a file. This output data is split into multiple files, each file containing the filament data at a single timestep. Output data is defined at the endpoints of the vortex filaments.

The output filenames have the format \path{[run name]_WakeData_[timestep number].csv}. The columns are described in \Cref{tbl:output_vortex_filaments}.
...
If \path{ProbeFlag} = 1, the velocities at the probes specified in the file at \path{ProbeSpecPath} will be computed and written to a file. Each probe output file has the format \path{probe_[probe number].csv}.

The columns are described in \Cref{tbl:output_probe_file}.

Each probe file has a header, described in \Cref{tbl:output_probe_file_header}, containing the location of the probe. This header is followed by the probe time series data, which is described in \Cref{tbl:output_probe_file}.
