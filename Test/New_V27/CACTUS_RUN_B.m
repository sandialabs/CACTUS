...
%     matlabpool open 11
% end
workingDir = '/Users/clkell/Documents/Modeling_Tools/CACTUS_gfortran_ck/Test/New_V27';
cd(workingDir);
global beta_off  ix R CRr rB bTwist HubRR Tilt eta bCone bi NBlade af NElem;

%%%%%%%%%% Define wind turbine blade geometry
Im_vortex = importdata('Design_B_dec.dat','\t',1);
% Params
R = 44.2913; % radius (ft)
...
% f = strrep(f,char('xstop = x'),char(strcat({'xstop = '},char(num2str(wake_cutoff(ix))))));
...
tic
% Run CACTUS !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
cmd = char(strcat({'cactus_ck '},inputfile{i}));
system(cmd);
toc
pause(0.1);
...
filename_wake = strcat('INPUT',num2str(i),'_WakeDefData.csv');
filename_wake_el = strcat('INPUT',num2str(i),'_WakeData.csv');

delimiterIn = ',';
headerlinesIn = 1;
import = importdata(filename_rev,delimiterIn,headerlinesIn);
importel = importdata(filename_el,delimiterIn,headerlinesIn);
import_wake = importdata(filename_wake,delimiterIn,headerlinesIn);
import_wake_el = importdata(filename_wake_el,delimiterIn,headerlinesIn);
DATA{1,i} = import.data(end,:);
DATA_ALL{1,i} = import.data;
DATA_ELEMENT{1,i} = importel.data;
DATA_WAKE{1,i} = import_wake.data;
DATA_WAKE_EL{1,i} = import_wake_el.data;
% Delete CACTUS generated files
delete(inputfile{i},geomfile{i},filename_rev,filename_param,filename_time,filename_el,filename_wake,filename_wake_el);
...
save('CACTUS_OUT_UWAKE_LongB','DATA_ALL')

% LARGE FILES
save('CACTUS_OUT_ELEM_CLEAN_UWAKE_LongB','DATA_ELEMENT','-v7.3') 

save('CACTUS_OUT_UWAKE_LongB','DATA_WAKE')


save('CACTUS_OUT_WAKE_EL_CLEAN_UWAKE_LongB','DATA_WAKE_EL')
